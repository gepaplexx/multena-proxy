local k = import 'github.com/grafana/jsonnet-libs/ksonnet-util/kausal.libsonnet';
{
  ...
  ns_proxy: {
      deployment: deployment.new(
        ...
        replicas=1,
        containers=[
          container.new($.config.ns_proxy.name, 'ghcr.io/gepaplexx/namespace-proxy:latest')
          + container.withPorts([port.new('http', $.config.ns_proxy.port)])
          + container.withEnvMap({
            'UPSTREAM_URL': $.prom_label_proxy.service.metadata.name+"."+"grafana-operator-system"+"svc.cluster.local:"+$.config.prom_label_proxy.port,
...
//          + securityContext.capabilities.withAdd("SYS_PTRACE")
  ...
  prom_label_proxy: {
        ...
        container.new("prom-label-proxy", "ghcr.io/lucostus/prom-label-proxy:multi-value-regex-v0.0.1")
        + container.withPorts([port.new('http', 9095)])
        + container.withEnvMap({
                      'insecure-listen-address': "127.0.0.1:"+$.config.prom_label_proxy.port,

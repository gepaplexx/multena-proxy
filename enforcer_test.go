...
var enforceTests = []enforceTest{
	...
	{`sum(rate({level="error", kubernetes_host="worker21", kubernetes_namespace_name="gepaplexx-cicd-tools"} [$__interval]))`, []string{"tenant1", "tenant2"}, ``, errors.New("unauthorized labels")},
	{`sum(rate({level="error", kubernetes_host="worker21", kubernetes_namespace_name=~"gepaplexx-cicd-tools|tenant1"} [$__interval]))`, []string{"tenant1", "gepaplexx-cicd-tools", "tenant3", "tenant_blubb"}, ``, errors.New("temporary not supported")},
	...
	{"sum by(level) (rate({kubernetes_namespace_name=\"multena\", level=~\"error|default\"} |= \"message\" | json | line_format \"{{.message}}\" |= \"message\" | json | unpack | line_format \"{{.message}}\" [$__interval]))", []string{"tenant2", "tenant3"}, "", errors.New("unauthorized labels")},
	...
	{"sum by(level) (rate({kubernetes_namespace_name=~\"multena|grafana\", level=~\"error|default\"} |= \"message\" | json | line_format \"{{.message}}\" |= \"message\" | json | unpack | line_format \"{{.message}}\" [$__interval]))", []string{"multena"}, "", errors.New("unauthorized labels")},
	...
	{`sum(rate({kubernetes_namespace_name=~"cert-manager|erhard-pg"} | json | line_format "{{.status_code}}" [$__interval])) > 2`, []string{"cert-manager"}, ``, errors.New("temporary not supported")},
	{`sum(rate({kubernetes_namespace_name=~"cert-manager|erhard-pg|default"} | json | line_format "{{.status_code}}" [$__interval])) > 2`, []string{"cert-manager|erhard-pg"}, ``, errors.New("temporary not supported")},
	{`sum(rate({kubernetes_namespace_name=~"cert-manager|erhard-pg"} | json | line_format "{{.status_code}}" [$__interval])) > 2`, []string{"tenant1"}, "", errors.New("temporary not supported")},
	{`sum(rate({kubernetes_namespace_name=~"cert-manager|erhard-pg"} | json | line_format "{{.status_code}}" [$__interval])) > 2`, []string{"different_namespace"}, "", errors.New("temporary not supported")},
	...
	{"sum by(level) (rate({kubernetes_namespace_name=\"multena\", level=~\"error|default\"} | json | line_format \"{{.message}}\" | json | line_format \"{{.message}}\" | __error__ = \"\" [$__interval])) - sum by(level) (rate({kubernetes_namespace_name=\"multena\", level=~\"error|default\"} | json | line_format \"{{.message}}\" | json | line_format \"{{.message}}\" | __error__ = \"\" [$__interval]))", []string{"grafana"}, "", errors.New("unauthorized labels")},
...
func TestEnforceNamespace(t *testing.T) {
	...
	for i, tt := range enforceTests {
		t.Run(tt.query, func(t *testing.T) {
			...
			write := false
			if fmt.Sprintf("%s", err) == "temporary not supported" {
				return // skip test
			}
